<header>
    ...
    <nav>
        ...
        @can('employees.view')
            <a href="{{ route('employees.index') }}" class="nav-btn {{ request()->routeIs('employees.*') ? 'active' : '' }}">👤 Cadastro</a>
        @endcan

        <a href="{{ route('profile.edit') }}" class="nav-btn {{ request()->routeIs('profile.*') ? 'active' : '' }}">⚙️ Perfil</a>
    </nav>
